...
class Attention(nn.Module):
    ...
    def forward(self, x, freqs=None, attn_mask=None):
        ...
        if self.is_causal:
            causal_mask = torch.ones(T, T, device=x.device, dtype=torch.bool).tril(diagonal=0)
        ...
        if self.return_attention:
            if attn_mask is not None:
                attn_mask_float = torch.zeros(attn_mask.shape, device=x.device, dtype=q.dtype).masked_fill(~attn_mask, -float('inf'))
                attn_weight = torch.softmax((q @ k.transpose(-2, -1) / math.sqrt(q.size(-1))) + attn_mask_float, dim=-1)
            else:
                attn_weight = torch.softmax((q @ k.transpose(-2, -1) / math.sqrt(q.size(-1))), dim=-1)
            return attn_weight
        # efficient attention using Flash Attention CUDA kernels
        y = torch.nn.functional.scaled_dot_product_attention(
            q, k, v, attn_mask=attn_mask, dropout_p=self.attn_drop if self.training else 0, is_causal=False)
...
class Block(nn.Module):
    ...
    def forward(self, x, freqs=None, attn_mask=None):
        y = self.attn(self.norm1(x), freqs, attn_mask=attn_mask)
...
class EEGTransformerReconstructor(nn.Module):
    ...
    def forward(self, x, use_out_proj=False, attn_mask=None):
...
class EEGTransformer(nn.Module):
    ...
    def load_pretrained(self, checkpoint_path):
        ...
        self.load_state_dict(target_encoder_stat)
...
class EEGPTClassifier(nn.Module):
    ...
    def forward_features(self, x, chan_ids=None, attn_mask=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        if chan_ids is None:
        ...
        if attn_mask is not None:
            patch_size = self.target_encoder.patch_embed.patch_size
            patch_stride = self.target_encoder.patch_embed.patch_stride
            if patch_stride is None:
                patch_stride = patch_size
            
            unfolded_mask = attn_mask.unfold(-1, patch_size, patch_stride)
            patch_mask = unfolded_mask.any(dim=-1)
        else:
            patch_mask = None

        if not self.use_predictor:
            x = self.reconstructor(x, self.use_out_proj, attn_mask=patch_mask)
        else:
            x = self.predictor(x, self.use_out_proj, attn_mask=patch_mask)
        ...
        if self.fc_norm is not None:
            ...
            if return_patch_tokens:
                ...
                if patch_mask is not None:
                    mask = patch_mask.repeat_interleave(self.target_encoder.embed_num, dim=1)
                    mask = mask.unsqueeze(-1).to(t.device)
                    return self.fc_norm((t * mask).sum(dim=1) / mask.sum(dim=1).clamp(min=1))
                else:
    ...
    def forward(self, x, chan_ids=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        attn_mask = ~torch.isnan(x).any(dim=1)
        x[torch.isnan(x)] = 0
        ...
        if (self.desired_time_len>0) and self.desired_time_len != x.shape[-1]:
            ...
            attn_mask = temporal_interpolation(attn_mask.unsqueeze(1).float(), self.desired_time_len, mode='nearest').squeeze(1).bool()

        x = self.forward_features(x, chan_ids=chan_ids, attn_mask=attn_mask, return_patch_tokens=return_patch_tokens, return_all_tokens=return_all_tokens, **kwargs)
        x = self.head(x)
        return x
